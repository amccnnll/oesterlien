# heat map of NEE from Østerlien 2014
# version 1.0
# alistair mcconnell
...
NEE_heatmap_14 <- ggplot(Oesterlien_C_R_2014,
                         aes(x = DoY,
                             y = Hour,
                             fill = NEE_f)) +
  geom_tile(na.rm = T) +
  ...
  geom_vline(xintercept = 130, color = "black", linetype = "dashed", size = 1) +
  geom_vline(xintercept = 269, color = "black", linetype = "dashed", size = 1) +
  ...
  annotate("text", x = 14, y = 22, label = "(2014)", fontface = 2)

# set colourbar to mach plot height
panel_height = unit(1,"npc") - sum(ggplotGrob(NEE_heatmap_14)[["heights"]][-3]) - unit(1,"line")

NEE_heatmap_14 <- NEE_heatmap_14 + guides(fill= guide_colourbar(barheight = panel_height,
